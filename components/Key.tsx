...
export const Key = ({ letter, className, status, passToGame }: KeyProps) =>  <span 
  onClick={() => status !== 0 ? passToGame(letter) : ""}
  // className={`py-3 px-2.5 lg:py-3 lg:px-4 bg-slate-300 hover:bg-slate-200 text-black rounded-md ${className}`}>
  className={`cursor-pointer grow overflow-hidden w-3 text-center flex flex-col justify-center 
      ${status === 0 ? "bg-gray-400" : status === 1 ? "bg-yellow-500" : status === 2 ? "bg-green-500" : "bg-slate-300 hover:bg-slate-200"}  
